...
protocol CellSearchModelRepresentable {
    var viewModel: CellNewsSearchIdentifiableProtocol? { get set }

}

class SearchCell: UITableViewCell, CellSearchModelRepresentable {

    var viewModel: CellNewsSearchIdentifiableProtocol? {
        didSet {
    ...
    func configure(unbrandering: Bool) {
        guard let viewModel = viewModel as? NewsSearchCellViewModel else { return }
